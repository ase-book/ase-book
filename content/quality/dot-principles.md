...
`.principles` provides three commands: `dot-scout` (setup), `dot-prime` (load principles before coding), and `dot-audit` (audit after). The hard part is not running an audit. The hard part is making the judgment persist through a team calendar.
